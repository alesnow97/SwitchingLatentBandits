...
class RegretMovielensFigure2b:

    def __init__(self, switching_env, num_experiments, sliding_window_size, epsilon,
                 exp3S_gamma, exp3S_normalization_factor, exp3S_limit,
                 num_particles, lowest_prob, num_lowest_prob,
    ...
    def generate_dirs(self):
        utils.set_base_path()
        dir_name = f"experiments/movielens_figure_2b/{self.switching_env.num_states}states_{self.switching_env.num_actions}actions_{self.switching_env.num_obs}obs"
        if os.path.exists(dir_name):
            os.chdir(os.path.join(os.getcwd(), dir_name))
            self.bandit_dir_path = os.path.join(os.getcwd(), 'bandit0')
            self.new_exp_index = len(os.listdir(self.bandit_dir_path))
        else:
    ...
    def run(self, total_horizon, delta,
