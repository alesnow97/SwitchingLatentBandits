...
from policies.our_policy_figure_2 import OurPolicyFigure2
...
class RegretFigure2a:

    def __init__(self, switching_env: SwitchingBanditEnv,
                 num_experiments, sliding_window_size, epsilon,
    ...
    def generate_dirs(self):
        utils.set_base_path()
        dir_name = f"experiments/regret_figure_2a/{self.switching_env.num_states}states_{self.switching_env.num_actions}actions_{self.switching_env.num_obs}obs"
        if os.path.exists(dir_name):
            os.chdir(os.path.join(os.getcwd(), dir_name))
            if self.loaded_bandit:
                self.bandit_dir_path = os.path.join(os.getcwd(),
                                                    f'bandit{self.bandit_num}')
    ...
    def run(self, total_horizon, exploration_horizon,
        ...
        algorithms_to_use = ['sliding_w_UCB', 'epsilon_greedy', 'exp3S', 'particle_filter', 'our_policy']
        ...
        sliding_window_size = int(2 * np.sqrt(np.log(total_horizon) * total_horizon / 2))
        ...
        for n in range(self.num_experiments):
            ...
            our_policy = OurPolicyFigure2(switching_env=self.switching_env,
        ...
        if self.new_bandit_index is not None:
            plt.title(f"New Bandit num is {self.new_bandit_index}")
        else:
            plt.title(f"Stored Bandit num is {self.bandit_num}")
