...
class OurPolicyFigure1b:
    ...
    def __init__(self, switching_env: SwitchingBanditEnv,
        ...
        self.min_estimation_errors = np.empty((self.num_checkpoints, 2))
        self.max_estimation_errors = np.empty((self.num_checkpoints, 2))
        self.k = 0
    ...
    def choose_arm(self):
        ...
        return self.min_flatten_combinations_of_pairs_of_arms[combination], self.max_flatten_combinations_of_pairs_of_arms[combination]

    def update(self, min_pulled_arm, min_observed_reward_index, max_pulled_arm, max_observed_reward_index):
        self.min_action_reward_list.append((min_pulled_arm, min_observed_reward_index))
        self.max_action_reward_list.append((max_pulled_arm, max_observed_reward_index))
        self.t += 1
    ...
    def estimate_transition_matrix(self):
        for k in range(self.k, int(self.t / 2)):
            ...
            self.num_arms_combinations_pulls[min_first_arm, min_second_arm] += 1

            max_first_arm, max_first_reward = self.max_action_reward_list[2 * k]
            max_second_arm, max_second_reward = self.max_action_reward_list[2 * k + 1]
            max_index = max_first_arm * self.num_actions * self.num_obs ** 2 + max_second_arm * self.num_obs ** 2 + max_first_reward * self.num_obs + max_second_reward
            self.max_count_vector[int(max_index)] += 1
            self.num_arms_combinations_pulls[max_first_arm, max_second_arm] += 1
        ...
        one_norm = abs(np.sum(distance_matrix))

        # new version computing the Frobenious norm
        frobenious_norm = np.sqrt(np.sum(distance_matrix**2))
        print(f"One norm error is {one_norm}")
        print(f"Frobenious norm is {frobenious_norm}")
        ...
        self.min_estimation_errors[update_index] = [self.t, frobenious_norm]


        masked_count_vector = self.max_count_vector[self.max_selected_mask]
        reshaped_count_vector = masked_count_vector.reshape(-1, self.num_obs ** 2)
        sum_reshaped_count_vector = reshaped_count_vector.sum(axis=1)
        sum_over_obs = sum_reshaped_count_vector.reshape((1, -1))
        normalizing_vec = np.tile(sum_over_obs.transpose(),
        ...
        flatten_transition_distribution = np.linalg.lstsq(self.reference_matrix[self.max_selected_mask], self.expected_visit_vector, rcond=None)[0]
        flatten_transition_distribution = flatten_transition_distribution / flatten_transition_distribution.sum()

        transition_stationary_distribution = flatten_transition_distribution.reshape((self.num_states, self.num_states))
        self.max_transition_matrix = transition_stationary_distribution / \
                                 transition_stationary_distribution.sum(axis=1)[:, None]
        print(f"Estimated transition matrix is \n{self.max_transition_matrix}")
        print(f"Real transition matrix is \n{self.real_transition_matrix}")

        distance_matrix = np.absolute(self.max_transition_matrix.reshape(-1) -
            self.real_transition_matrix.reshape(-1))
        ...
        update_index = int((self.t - self.starting_checkpoint) / self.checkpoint_duration)
        self.max_estimation_errors[update_index] = [self.t, frobenious_norm]
