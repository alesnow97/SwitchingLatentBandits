...
class OurPolicyFigure2:
    ...
    def estimate_transition_matrix(self):
        ...
        # check that all the elements of the transition matrix are positive
        if np.any(self.transition_matrix[self.transition_matrix < 0]):
    ...
    def compute_exploration_arms(self, num_selected_actions):
        if (num_selected_actions >= self.num_actions) or \
            ...
            masks = []
            value_to_minimize = []
            # print(len(possible_action_subsets))

            for j, subset in enumerate(possible_action_subsets):
                ...
                # print(s)
                if j % 100 == 0:
                    print(f"{j} possible arms combinations have been tried")
