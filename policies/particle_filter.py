...
class ParticleFilter:
    ...
    def __init__(self, switching_env: SwitchingBanditEnv, total_horizon,
    ...
    def reset(self):
        ...
        if self.dirichlet_prior is not None:
            ...
            masked_dirichlet_prior = np.where(dirichlet_prior < 1, 1, dirichlet_prior)
            dirichlet_prior = masked_dirichlet_prior
        else:
    ...
    def choose_arm(self):
        ...
        for particle in range(self.num_particles):
            action_obs_tensor += (self.weights[particle] *
                                  self.reward_observation_matrix[
    ...
    # def start_parallel_update(self, pulled_arm, observed_reward_index):
    #     start_time = time.time()
    #     result = Parallel(n_jobs=16)(delayed(self.update_parallel)(particle, pulled_arm, observed_reward_index) for particle in range(self.num_particles))
    #
    #     end_time = time.time()
    #     print(f"Time to update is {end_time - start_time}")

    def update(self, pulled_arm, observed_reward_index):
        # parallel = True
        #
        # if parallel:
        #     self.start_parallel_update(pulled_arm, observed_reward_index)
        # else:
    ...
    #             # if self.dirichlets[i].sum() > 1000 * self.num_states**2:
    #             # if j % 2 == 0:
    #             random = np.random.random()
    #             new_sample = (
    #                         self.real_transition_matrix * 10 + random * self.t / 1000).astype(
    #                         # self.real_transition_matrix * 100).astype(
    #                 int)
    #             new_sample = np.clip(new_sample, a_min=1,
    #                                       a_max=None)
    #             # else:
    #             #     new_sample = np.clip(self.dirichlets[i] // 1.5, a_min=1, a_max=None)
    #             new_dirichlets.append(new_sample)
    #             # else:
        ...
        for i, sample in enumerate(sampled_particles):
            for j in range(sample):
                if np.random.random() > 0.0:
                    new_sample = (self.real_transition_matrix * 50 + self.t / 1000).astype(int)
                    new_sample = np.clip(new_sample, a_min=1, a_max=None)
                    new_dirichlets.append(new_sample)
                    new_weights.append(1 / self.num_particles)
                else:
                    new_dirichlets.append(self.dirichlets[i])
                    new_weights.append(self.weights[i])
                new_current_states.append(self.current_states[i])
